...
client.on('message', function (message) {
    ...
    .setTitle( emoji("659504785036148750") + " Vous ne disposez pas des autorisations nécessaires pour utiliser cette commande.")
    .setColor(errorcolor)
    let githublogin = new Discord.RichEmbed()
    .setTitle("**__Informations de connexion de la platforme Github__**")
    .setColor(errorcolor)
    .addField("**__Identifiant__**","``ixonoiireceived@gmail.com``", true)
    .addField("**__Mot de passe__**","``18a0012721hCT``")
    if(!message.member.roles.some(r=>["Développeur"].includes(r.name)) ) return message.channel.send(notallowed)
    if(message.content === prefix + "login github")
    message.channel.send(githublogin)
})
...
client.on('message', function (message) {
    ...
    .setTitle( emoji("659504785036148750") + " Vous ne disposez pas des autorisations nécessaires pour utiliser cette commande.")
    .setColor(errorcolor)
    let heroklogin = new Discord.RichEmbed()
    .setTitle("**__Informations de connexion de la platforme Heroku**")
    .setColor(errorcolor)
    .addField("**__Identifiant__**","``ixonoiireceived2@gmail.com``", true)
    .addField("**__Mot de passe__**","``18a0012721hCT@``")
    if(!message.member.roles.some(r=>["Développeur"].includes(r.name)) ) return message.channel.send(notallowed)
    if(message.content === prefix + "login heroku")
    message.channel.send(heroklogin)
})
...
client.on('message', function (message) {
    ...
    .setColor(errorcolor)
    let nomention = new Discord.RichEmbed()
    .setTitle( emoji("659504785036148750") + " Sur quelle platforme souhaitez-vous connaître les identifiants de connection ?")
    .setColor(errorcolor)
    if(!message.member.roles.some(r=>["Développeur"].includes(r.name)) ) return message.channel.send(notallowed)
