...
client.on('message', function (message) {
    ...
    if (args[0].toLocaleLowerCase() === prefix + 'ban') {
        ...
        .setTitle(member + ' à été banni du serveur pour la raison suivante : ' + reason)
        .setColor(couleur)
        let nobanperm = new Discord.RichEmbed()
        .setTitle("Je ne peux pas bannir cet utilisateur.")
        .setColor(couleur)
        if(!message.member.roles.some(r=>["Modérateur","Administrateur","Super Administrateur"].includes(r.name)) ) return message.channel.send(erreurperm)
       let member = message.mentions.members.first()
       let reason = args.slice(2).join(" ")
       if (!member) return message.channel.send(nomention)
       if (!reason) return message.channel.send(noreason)
       if (member.highestRole.calculatedPosition >= message.member.highestRole.calculatedPosition && message.author.id !== message.guild.owner.id) return message.channel.send(nobanperm)
       if (!member.bannable) return message.channel.send(nobanperm)
       ...
       .addField("**__Utilisateur:__**", message.author.tag + " | " + message.author.id)
       .addField("**__Serveur:__**", message.guild.name + " | " + message.guild.id)
       .addField("**Membre banni :__**", member)
       .addField("**Raison :__**", reason)
